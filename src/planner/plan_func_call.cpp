...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower") {
    if (args.size() != 1) {
      throw Exception(fmt::format("func call {} should have 1 argument", func_name));
    }
    return std::make_unique<StringExpression>(std::move(args[0]), StringExpressionType::Lower);
  } else if (func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("func call {} should have 1 argument", func_name));
    }
    return std::make_unique<StringExpression>(std::move(args[0]), StringExpressionType::Upper);
  }
