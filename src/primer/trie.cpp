...
auto Trie::Put(std::string_view key, T value) const -> Trie {
  ...
  if (root_ == nullptr) {
    ...
    if (key.empty()) {
      ...
      auto sub_tree = Trie(nullptr);
      if (root_->children_.count(key[0]) != 0) {
        sub_tree = Trie(root_->children_.at(key[0]));
      }
      auto new_sub_tree = sub_tree.Put(key.substr(1), std::move(value));
      auto new_root = root_->Clone();
      new_root->children_[key[0]] = new_sub_tree.root_;
      return Trie(std::move(new_root));
    }
  }
